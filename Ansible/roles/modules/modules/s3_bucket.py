...
requirements (for s3 bucket encryption):
    - boto3 >= 1.4.8
    - python >= 2.6
options:
  force:
  ...
  encryption:
    ...
    default: absent
    choices: [ 'present', 'absent' ]
  serverside_encryption_configuration:
    description:
      - Container for server-side encryption configuration rules. Currently S3 supports one rule only.
    required: true if encryption == enabled
    default: null
  acl:
    description:
...
# Create a bucket with encryption enabled and set by default
- s3_bucket:
    name: mys3bucket
    description: present
    serverside_encryption_configuration: {'Rules': [{'ApplyServerSideEncryptionByDefault': {'SSEAlgorithm': 'aws:kms', 'KMSMasterKeyID': 'string'}},]}

# Create a bucket with private ACL
- s3_bucket:
    name: mys3bucket
    description: present
    acl: private
...
def _create_or_update_bucket(connection, module, location):
    ...
    serverside_encryption_configuration = module.params.get("serverside_encryption_configuration")
    ...
    # Set up boto3 connection for ACL and Encryption
    if module.params.get('encryption') == 'present' or acl:
        region, ec2_url, aws_connect_params_3 = get_aws_connection_info(module, boto3=True)
        try:
            connection_3 = boto3_conn(module, conn_type='client', resource='s3', region=region, endpoint=ec2_url, **aws_connect_params_3)
        except botocore.exceptions.NoRegionError:
            module.fail_json(msg="Region must be set in parameters, environment variables or boto3 configuration")
        
    # Encryption
    # Making some blind assumption that by the time we get here the bucket actually exists and all.
    if module.params.get('encryption') == 'present':
        response_3 = connection_3.put_bucket_encryption(Bucket=name,
                                                        ServerSideEncryptionConfiguration=serverside_encryption_configuration
                                                        )

    # ACL
    if acl:
        response_3 = connection_3.put_bucket_acl(Bucket=name, ACL=acl)
    
    module.exit_json(changed=changed, name=bucket.name, versioning=versioning_status,
                     requester_pays=requester_pays_status, policy=current_policy,
                     tags=current_tags_dict, encryption=encryption, acl=acl)
...
def main():
    ...
    argument_spec.update(
        dict(
            ...
            encryption=dict(required=False, default='absent', type='str', choices=['present', 'absent']),
            serverside_encryption_configuration=dict(required=False, default={}, type='dict'),
            acl=dict(required=False, default=None, type='str'),
        )
    )
    ...
    if not HAS_BOTO_3 and module.params.get('encryption') == 'present':
        module.fail_json(msg='boto3 required for encryption')
    
    if not StrictVersion(boto3.__version__) >= StrictVersion("1.4.8"):
        module.fail_json(msg='boto3 >= 1.4.8 required for encryption')
