...
def main():
    ...
    if role.startswith('arn:aws:iam'):
        ...
        try:
            ...
            # -----------------------------------------------------------------
            # MONKEY PATCH
            # the provided version of the lambda module was using the following
            # call to get the account id 
            #
            # iam_client.get_user()['User']['Arn'].split(':')[4]
            #
            # using sts, this cause the module to throw an error that it was
            # unable to get the user so for now, we have patched the module
            # locally until it is fixed upstream
            #
            # @see https://github.com/ansible/ansible/issues/25878
